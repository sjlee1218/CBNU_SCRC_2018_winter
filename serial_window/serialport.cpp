...
bool CSerialPort::ReadString( char buf[40]) {
	DWORD dwBytesTransferred = 0;

	if (ReadFile(m_hComm, buf, 54, &dwBytesTransferred, 0)) {
		return true;
	}
	return false;
...
/*
bool CSerialPort::Read_payload(BYTE toRead[3], BYTE payload[92]) {
	if (ReadFile(m_hComm, resp, ))
}
*/